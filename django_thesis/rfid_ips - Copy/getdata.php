                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    if ($row = mysqli_fetch_assoc($resultl)){
                        ...
                        if ($row['device_uid'] == $device_uid || $row['device_uid'] == 0){
                                ...
                                $sql = "SELECT * FROM users_logs WHERE card_uid=? AND checkindate=? AND card_out=0 AND Macaddress=? AND Birthdate=? AND Contact=? AND EmergencyContact=? AND ValidationPeriod=? AND MedicalHistory=?";
                                $result = mysqli_stmt_init($conn);
                                if (!mysqli_stmt_prepare($result, $sql)) {
                                    ...
                                    mysqli_stmt_bind_param($result, "ssssssss", $card_uid, $d, $Macaddress, $Birthdate, $Contact, $EmergencyContact, $ValidationPeriod , $MedicalHistory);
                                    mysqli_stmt_execute($result);
                                    $resultl = mysqli_stmt_get_result($result);
                                    //*****************************************************
                                    //Login
                                    if (!$row = mysqli_fetch_assoc($resultl)){
                                        ...
                                        $sql = "INSERT INTO users_logs (username, serialnumber, card_uid, device_uid, device_dep, checkindate, timein, timeout, Macaddress, Birthdate, Contact , EmergencyContact, ValidationPeriod , MedicalHistory) VALUES (?,? ,?, ?, ?, ?, ?, ?, ?, ? , ? ,? ,? ,?)";
                                        $result = mysqli_stmt_init($conn);
                                        if (!mysqli_stmt_prepare($result, $sql)) {
                                            ...
                                            $timeout = "00:00:00";
                                            mysqli_stmt_bind_param($result, "sdssssssssssss", $Uname, $Number, $card_uid, $device_uid, $device_dep, $d, $t, $timeout, $Macaddress, $Birthdate, $Contact , $EmergencyContact , $ValidationPeriod , $MedicalHistory);
                                            mysqli_stmt_execute($result);
