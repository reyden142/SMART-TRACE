    ...
    if "already running" not in data:
        if connection:
            ...
            try:
                ...
                insert_query = "INSERT INTO position_scanner_cap2 (mac_address, ssid, channel, signal_strength, source, timestamp) VALUES (%s, %s, %s, %s, %s, %s)"
...
def main():
    ...
    # Create the command
    command = f'/caps-man/interface/scan cap2'
    ...
    while True:
        try:
            ...
            # Define the CAP interface names ('cap2' and 'cap2')
            cap_interfaces = ['cap2']
            ...
            if "failure: already running" not in output:  # Check the condition here as well
                ...
                if connection:
                    ...
                    your_query = "SELECT * FROM `position_scanner_cap2`"

                    # Use pandas to read the query result into a DataFrame
                    df = pd.read_sql_query(your_query, connection)

                    # Replace 'output_file.csv' with the desired file name
                    position_scanner_cap2 = 'position_scanner_cap2.csv'

                    # Save the DataFrame to a CSV file
                    df.to_csv(position_scanner_cap2, index=False)

                    print(f"Data extracted and saved to {position_scanner_cap2}")

                    # Read the CSV file into a new DataFrame
                    position_scanner_cap2 = pd.read_csv(position_scanner_cap2)

                    print('position_scanner_cap2', position_scanner_cap2)

                    # Check for three similar rows based on specific columns
                    columns_to_check = ['mac_address', 'ssid', 'channel', 'signal_strength']

                    duplicates = position_scanner_cap2[
                        position_scanner_cap2.duplicated(subset=columns_to_check, keep=False)]

                    if not duplicates.empty:
                        # Group by 'ssid', 'channel', and 'signal_strength' columns and count the number of duplicate rows for each group
                        duplicate_counts = position_scanner_cap2[
                            position_scanner_cap2.duplicated(subset=columns_to_check, keep=False)].groupby(
                        ...
                        for (ssid, channel, signal_strength), count in duplicate_counts.items():
                            ...
                            total_count = position_scanner_cap2[position_scanner_cap2['ssid'] == ssid].shape[0]
                            ...
                            if count >= 6:
                                ...
                                group_data = position_scanner_cap2[
                                    ...
                                    (position_scanner_cap2['channel'] == channel) &
                                    (position_scanner_cap2['signal_strength'] == signal_strength)
                                    ]

                                # Select the row with the maximum signal strength
                                #representative_row = group_data.loc[group_data['signal_strength'].idxmax()]

                                # Save the representative row to "scanned_aps_cap2.csv"
                                #representative_row.to_csv("scanned_aps_cap2.csv", index=False, mode='a', header=False)
                                ...
                                print(
                                    f"Data transferred to scanned_aps_cap2.csv for SSID: {ssid}, Channel: {channel}, Signal Strength: {signal_strength}")
    ...
    if connection:
        try:
            with connection.cursor() as cursor:
                delete_query = "DELETE FROM position_scanner_cap2 WHERE ssid = %s"
