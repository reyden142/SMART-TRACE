...
def addBridgeInterface():
    ...
    response = requests.put(apiURL + '/interface/bridge', auth=HTTPBasicAuth(apiUsername, apiPassword), verify=False,
                            data=json.dumps({"name": newBridgeName}))
...
def addIPaddressInterface():
    ...
    while goodIP:
        try:
            getIPAddress = input(
                "Enter the IP Address you want to assign to " + interfaceName + " use format(A.B.C.D/E, e.g. 192.168.0.1/24):")
            ...
            for ipQuery in response.json():
                if str(getIPAddress) == ipQuery["address"]:
                    goodIP = True
                    input("This IP address is already in use with the interface " + ipQuery[
                        "interface"] + " press ENTER to continue to assigna new IP address...")
                    break
    ...
    response = requests.put(apiURL + '/ip/address', auth=HTTPBasicAuth(apiUsername, apiPassword), verify=False,
                            data=json.dumps({"interface": interfaceName, "disabled": "false", "address": getIPAddress}))
